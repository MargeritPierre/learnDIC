#%% Import useful libraries
import os
import numpy as np
from scipy import interpolate
from scipy import sparse
from scipy.sparse.linalg import spsolve
from scipy.sparse.linalg import lsqr
import matplotlib.pyplot as plt
import matplotlib.image as mpimg
import matplotlib.tri as mptri

#%% PLOT FUNCTIONS
def plotImg(I):
    I = np.array(I)
    I = np.repeat(I[:,:,np.newaxis],3,2) # three colors
    plt.imshow(I)
def plotMesh(x,elems,colorData=np.array([]),edgeColor = 'black'):
    plt.fill(x[elems,0].transpose(),x[elems,1].transpose(),edgecolor=edgeColor,fill=False)
    #plt.scatter(Xg[:,0],Xg[:,1],c = "k")
    if colorData.size==0: return 
    tri = np.vstack((elems[:,[0,1,2]],elems[:,[2,3,0]]))
    tri = mptri.Triangulation(x[:,0], x[:,1], tri)
    plt.tricontourf(tri,colorData.flatten())

#%% LOAD IMAGES *************************************************
# Change this folder to fit your configuration
folder = 'D:/POSTDOC/04_TEACHING/ENPC/DMS_Identification/DM2/Images'
# Load images
files = os.listdir(folder)
IMG = [mpimg.imread(folder + '/' +  name) for name in files]
# Show images
for img in IMG:
    plotImg(img)
    plt.show()
# Image info
Np1 = IMG[0].shape[1]
Np2 = IMG[0].shape[0]
nPixels = Np1*Np2
# Pixel coordinates
Xp1 = np.arange(0,Np1)
Xp2 = np.arange(0,Np2)
Xp1,Xp2 = np.meshgrid(Xp1,Xp2)

#%% CREATE THE GRID MESH *************************************************
# mesh: a list of Nodes Xg and a list of quad elements 
# Grid parameters
margin = 50 ;
xmin = margin ; xmax = Np1-margin ; dx = 25
ymin = margin ; ymax = Np2-margin ; dy = dx
# Node coordinates
Xg1 = np.arange(xmin,xmax+1,dx)
Xg2 = np.arange(ymin,ymax+1,dy)
Xg1,Xg2 = np.meshgrid(Xg1,Xg2)
Ng1 = Xg1.shape[1]
Ng2 = Xg2.shape[0]
# flatten
Xg = np.array([Xg1.flatten(),Xg2.flatten()]).transpose()
nNodes = Xg.shape[0]
# Quads
p0 = np.arange(0,Ng1)[np.newaxis,:] + Ng1*np.arange(0,Ng2)[:,np.newaxis]
p0 = p0[0:-1,0:-1].flatten()
quads = p0[:,np.newaxis] + np.array([0,1,Ng1+1,Ng1])
nElems = quads.shape[0]
# plot
plotImg(IMG[0])
plotMesh(Xg,quads)
plt.show()

#%% GET PIXELS IN EACH QUAD *******************************************
# create a SPARSE matrix IN of shape (nPixels,nElems)
pp = [] # linear pixel index
ee = [] # element index
# Get linear pixel indices included in each quad
for q in range(nElems):
    xq = Xg[quads[q,:],:] # quad point coordinates
    box = np.array([np.min(xq,0),np.max(xq,0)]) # quad bounding box
    p1 = np.arange(np.ceil(box[0,0]),box[1,0]) # horizontal indices
    p2 = np.arange(np.ceil(box[0,1]),box[1,1]) # vertical indices
    pl = (p1[np.newaxis,:] + Np1*p2[:,np.newaxis]).flatten() # linear indices
    pp.append(pl)
    ee.append(np.tile(q,pl.shape))
# concatenate all indices
pp = np.hstack(pp) ;
ee = np.hstack(ee) ;
vv = np.tile(1.0,pp.shape)
IN = sparse.csr_matrix((vv,(pp,ee)),(nPixels,nElems))
# Region Of Interest
ROI = np.sum(IN,1)
# show the ROI
plotImg(ROI.reshape((Np2,Np1)))

#%% DELETE ELEMENTS *************************************************
# delete elements with a low contrast between included pixels
minStd = 0.18 # minimum stantard deviation in each element
# gray value of each pixel in each element
img = IN.multiply(IMG[0].flatten()[:,np.newaxis])
# mean gray value in the element
meanOverElem = np.sum(img,0)/np.sum(IN,0)
# zero mean image
img_zeromean = img-IN.multiply(meanOverElem)
# standard deviation in each element
stdOverElem = np.sqrt(np.sum(img_zeromean.power(2),0)/(dx*dy))
# deletion criterion
highContrast = np.where(stdOverElem>=minStd)[1]
print('Elements deleted: ' + str(nElems-highContrast.size))
# Delete elements
quads = quads[highContrast,:]
IN = IN[:,highContrast]
ROI = np.sum(IN,1)
nElems = quads.shape[0]
# delete unused nodes
nn, qq = np.unique(quads.flatten(), return_inverse=True)
Xg = Xg[nn,:]
quads = qq.reshape((nElems,4))
nNodes = nn.size
# plot the valid elements
plotImg(IMG[0])
plotMesh(Xg,quads)
plt.show()

#%% BUILD THE MATRIX OF SHAPE FUNCTIONS *******************************************
# create a SPARSE matrix N of shape (nPixels,nNodes) containing the FEM shape functions
# bilinear quad shape function, for local coordinates xi in [0,1]²
# first determine the local coordinates of each pixel in its corresponding element
p0 = Xg[quads[:,0],:] # upper-left corner point
p0 = IN.dot(p0) # project for each pixel of the ROI
pl = np.arange(Np1*Np2) # linear piwel coordinates
(p2,p1) = divmod(pl,Np1) ; # pixel coordinates
pp = np.array([p1,p2]).transpose() # pixel coordinates 
xi = (pp-p0)/[dx,dy] # LOCAL coordinates xi in [0,1]²
# create the shape functions
N0 = (1-xi[:,0])*(1-xi[:,1]) # upper-left corner
N1 = xi[:,0]*(1-xi[:,1]) # upper-right corner
N2 = xi[:,0]*xi[:,1] # lower-right corner
N3 = (1-xi[:,0])*xi[:,1] # lower-left corner
# Build the sparse matrix
ee = IN.dot(np.arange(0,nElems)).astype(int) # associated element
pp = np.tile(pl,(1,4)).flatten() # pixel indices
nn = quads[ee,:].transpose().flatten() # node indices
vv = np.hstack((N0,N1,N2,N3)) # shape function values
N = sparse.csr_matrix((vv,(pp,nn)),(nPixels,nNodes))
# Display a function
fn = 0.5+0.5*np.sin(2*np.pi*Xg[:,0]/dx*0.1)*np.sin(2*np.pi*Xg[:,1]/dy*0.2) # function value on nodes
fp = N.dot(fn) # function values at pixels
plotImg(fp.reshape((Np2,Np1)))

#%% Interpolation function *******************************************
# Linear interpolation of an Image i at real-valued coordinates x = (x1,x2)
def interp(i,x):
    X1 = np.arange(0,i.shape[1])
    X2 = np.arange(0,i.shape[0])
    f = interpolate.RegularGridInterpolator((X1,X2),i.transpose(),'linear',False,0)
    return f(x)

#%% Gradient function ************************************************
# using finite difference schemes
def d_dx1(I):
    return np.c_[I[:,1]-I[:,0] , 0.5*(I[:,2:]-I[:,0:-2]) , I[:,-1]-I[:,-2]]
def d_dx2(I):
    na = np.newaxis
    return np.r_[I[1,na]-I[0,na] , 0.5*(I[2:,:]-I[0:-2,:]) , I[-1,na]-I[-2,na]]
# Display the norm of the gradient of an image
I = IMG[0]
g = np.sqrt(d_dx1(I)**2 + d_dx2(I)**2)
plotImg(g)

#%% REGULARIZATION: SECOND GRADIENT ***************************************
# build SPARSE matrices G2 so that d²f/(dxj.dxk) = G[j][k]*fn
# d²f/(dxj.dxk) = d²(N*fn)/(dxj.dxk) = (d²N/(dxj.dxk))*fn so G[j][k] = dN/d(dxj.dxk)
# d²N_dxi1²
d2N0_dxi12 = 0 # upper-left corner
d2N1_dxi12 = 0 # upper-right corner
d2N2_dxi12 = 0 # lower-right corner
d2N3_dxi12 = 0 # lower-left corner
# d²N_dxi1dxi2
d2N0_dxi1dxi2 = 1 # upper-left corner
d2N1_dxi1dxi2 = -1 # upper-right corner
d2N2_dxi1dxi2 = 1 # lower-right corner
d2N3_dxi1dxi2 = -1 # lower-left corner
# d²N_dxi2²
d2N0_dxi22 = 0 # upper-left corner
d2N1_dxi22 = 0 # upper-right corner
d2N2_dxi22 = 0 # lower-right corner
d2N3_dxi22 = 0 # lower-left corner
# derivatives w.r.t x1 & x2 , shape: (nGP,4)
d2N_dx12 = np.vstack((d2N0_dxi12,d2N1_dxi12,d2N2_dxi12,d2N3_dxi12)).transpose()/dx**2
d2N_dx22 = np.vstack((d2N0_dxi22,d2N1_dxi22,d2N2_dxi22,d2N3_dxi22)).transpose()/dy**2
d2N_dx1dx2 = np.vstack((d2N0_dxi1dxi2,d2N1_dxi1dxi2,d2N2_dxi1dxi2,d2N3_dxi1dxi2)).transpose()/(dx*dy)
# build the sparse matrices indices, shape: (nElems,4)
vv11 = np.tile(d2N_dx12,(nElems,1))
vv22 = np.tile(d2N_dx22,(nElems,1))
vv12 = np.tile(d2N_dx1dx2,(nElems,1))
pp = np.tile(np.arange(nElems)[:,np.newaxis],(1,4))
nn = quads
# sparse matrices
matShape = (nElems,nNodes) ;
pp = pp.flatten()
nn = nn.flatten()
d2N_dx12 = sparse.csr_matrix((vv12.flatten(),(pp,nn)),matShape)
d2N_dx22 = sparse.csr_matrix((vv22.flatten(),(pp,nn)),matShape)
d2N_dx1dx2 = sparse.csr_matrix((vv12.flatten(),(pp,nn)),matShape)
# gradient matrices
G2 = [[d2N_dx12,d2N_dx1dx2],[d2N_dx1dx2,d2N_dx22]]
# check with known functions
G2[1][0]*(Xg[:,0]*Xg[:,1]) # d²(x1*x2)/(dx1.dx2) = 1

#%% Gauss-Newton algorithm ************************************************
# Error function J = 1/2*sum(r**2)
# Define the procedure
display = True
eps = 1e-4 # convergence criterion
maxIt = 100 # prevent infinite loops
beta = 1e6 # regularization parameter
I = IMG[0] # reference image
i = IMG[1] # current image
# Initialize
u = np.zeros((nNodes,2)) # nodal displacement to identify
# restrict to ROI
inROI = np.where(np.sum(IN,1)>0)[0]
Nr = N[inROI,:]
X = Nr.dot(Xg) # current configuration
for i in IMG[1:]:
    du = 1e10*np.ones((nNodes,2))
    it = 0
    while np.sqrt(np.sum(du**2))>eps and it<maxIt:
        it = it+1
        # Current configuration
        x = X + Nr.dot(u)
        # Compute the residual
        r = interp(i,x)-interp(I,X)
        # Interpolated image gradients
        di_dx1 = interp(d_dx1(i),x)
        di_dx2 = interp(d_dx2(i),x)
        # residual derivatives
        dr_du1 = Nr.multiply(di_dx1[:,np.newaxis])
        dr_du2 = Nr.multiply(di_dx2[:,np.newaxis])
        dr_du = sparse.hstack((dr_du1,dr_du2))
        # Jacobian j = dJ_du
        j = dr_du.transpose().dot(r)
        # Hessian H = d²J_du²
        H = dr_du.transpose().dot(dr_du)
        # Add the regularization
        gg = G2[1][0].transpose()*G2[1][0]
        Hr = sparse.vstack((sparse.hstack((gg,gg*0)),sparse.hstack((gg*0,gg))))
        jr = Hr*(u.transpose().flatten()[:,np.newaxis]).flatten()
        j = j + beta*jr
        H = H + beta*Hr
        # Displacement update
        du = spsolve(H,j)
        du = du.reshape((2,nNodes)).transpose()
        u = u - du
        # Display
        if display:
            print(np.sqrt(np.sum(du**2)))
            plotImg(i)
            plotMesh(Xg + u,quads,np.sqrt(np.sum(du**2,1)))
            plt.show()
        
#%% PLOT DATA ON THE MESH ************************************************
dataToPlot = u[:,1]
plotImg(i)
plotMesh(Xg + u,quads,dataToPlot,edgeColor='none')
plt.colorbar()

#%% FIRST GRADIENT & STRAINS ************************************************
# build SPARSE matrices G so that df/dxj = G[j]*fn
# df/dxj = d(N*fn)/dxj = (dN/dxj)*fn so G[j] = dN/dxj
# first we evaluate the derivatives at gauss points
xi = np.array([[1/4,1/4],[3/4,1/4],[3/4,3/4],[1/4,3/4]]) # gauss points local coordinates
nGP = xi.shape[0]
# shape functions at gauss points
N0gp = (1-xi[:,0])*(1-xi[:,1]) # upper-left corner
N1gp = xi[:,0]*(1-xi[:,1]) # upper-right corner
N2gp = xi[:,0]*xi[:,1] # lower-right corner
N3gp = (1-xi[:,0])*xi[:,1] # lower-left corner
Ngp = np.vstack((N0gp,N1gp,N2gp,N3gp)).transpose()
# derivative w.r.t xi1 at gauss points
dN0_dxi1 = -(1-xi[:,1]) # upper-left corner
dN1_dxi1 = (1-xi[:,1]) # upper-right corner
dN2_dxi1 = xi[:,1] # lower-right corner
dN3_dxi1 = -xi[:,1] # lower-left corner
# derivative w.r.t xi2 at gauss points
dN0_dxi2 = -(1-xi[:,0]) # upper-left corner
dN1_dxi2 = -xi[:,0] # upper-right corner
dN2_dxi2 = xi[:,0] # lower-right corner
dN3_dxi2 = (1-xi[:,0]) # lower-left corner
# derivatives w.r.t x1 & x2 , shape: (nGP,4)
dN_dx1 = np.vstack((dN0_dxi1,dN1_dxi1,dN2_dxi1,dN3_dxi1)).transpose()/dx
dN_dx2 = np.vstack((dN0_dxi2,dN1_dxi2,dN2_dxi2,dN3_dxi2)).transpose()/dy
# build the sparse matrices indices, shape: (nGP*nElems,4)
vv = np.tile(Ngp,(nElems,1))
vv1 = np.tile(dN_dx1,(nElems,1))
vv2 = np.tile(dN_dx2,(nElems,1))
pp = np.tile(np.arange(nGP*nElems)[:,np.newaxis],(1,4))
nn = np.repeat(quads,nGP,0)
# sparse matrices
matShape = (nGP*nElems,nNodes) ;
pp = pp.flatten()
nn = nn.flatten()
Ngp = sparse.csr_matrix((vv.flatten(),(pp,nn)),matShape)
dN_dx1 = sparse.csr_matrix((vv1.flatten(),(pp,nn)),matShape)
dN_dx2 = sparse.csr_matrix((vv2.flatten(),(pp,nn)),matShape)
# gradient matrices
iNgp = spsolve(Ngp.transpose()*Ngp,Ngp.transpose())
G = [iNgp*dN_dx1,iNgp*dN_dx2]
# Check with known functions
G[0].dot(Xg[:,0]) # dx1_dx1 = 1
G[0].dot(Xg[:,1]) # dx2_dx1 = 0
G[1].dot(Xg[:,0]) # dx1_dx2 = 0
G[1].dot(Xg[:,1]) # dx2_dx2 = 1
# Strain Matrix
O = sparse.csr_matrix((nNodes,nNodes))
# EPSILON = [[E11],[E22],[2*E12]] = B*[[u1],[u2]]
B11 = sparse.hstack((G[0],O))
B22 = sparse.hstack((O,G[1]))
B12 = sparse.hstack((G[1],G[0]))
B = sparse.vstack((B11,B22,B12))
        
#%% PLOT STRAINS ON THE MESH ************************************************
dataToPlot = B22.dot(u.transpose().flatten()[:,np.newaxis])
plotImg(i)
plotMesh(Xg + u,quads,dataToPlot,edgeColor='none')
plt.colorbar()















